...
export function modelForSlot(slot: OcrSlotName, env: Env): string {
  switch (slot) {
    case 'ollama-local':
      return env.ollamaVisionModel;
    case 'ollama-cloud':
      return env.ollamaCloudModel;
    case 'openrouter':
      return env.openrouterVisionModel;
    case 'openai-compatible':
      return env.openaiCompatibleModel ?? '';
  }
}
...
export async function runOcrPipeline(input: PipelineInput): Promise<PipelineOutcome> {
  ...
  const billedCostCents =
    input.provider.estimateCostCents() + failedAttemptCostCents(input.provider);
  ...
  if (!schema.ok) {
    ...
    return {
      ok: false,
      statusCode: 502,
      error: `schema invalid: ${schema.error}`,
      imageType,
      latencyMs: Date.now() - t0,
  ...
  const active = isChain
    ? ((input.provider as ChainOcrProvider).activeProvider ?? input.provider)
    : input.provider;
