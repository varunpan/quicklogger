...
| iOS Safari pump submission returns `400 multipart parse failed` (pre-fix) | Symptom: `image` part in the multipart wire body is zero bytes between headers and the next boundary. Two compounding causes — (a) `prefillDateFromPhoto(file)` racing against `resizeForOcr`'s `createImageBitmap(file)` on a PHAsset-backed HEIC File, (b) `OffscreenCanvas.convertToBlob` intermittently returning zero-byte Blobs on iOS Safari 16.4+. Fix: photos are buffered into memory via `bufferPickedPhoto` at pick time (eliminates concurrent-read race on the original File), and `resizeForOcr` detects zero-byte output from `convertToBlob` and falls back to `HTMLCanvasElement.toBlob` (separate decode/encode chain). | Two independent defensive fixes because static analysis can't pin which is the actual trigger on a given device. The fallback path emits `console.warn` (forwarded via `/api/log`) so we can tell from server logs whether `OffscreenCanvas` is still misbehaving in production. |
...
| Photo picked but `file.arrayBuffer()` reads zero bytes at pick time | `bufferPickedPhoto` returns null, toast shows `"Couldn't read photo — try again"`, no preview opens | Catches degenerate Files (revoked permissions mid-pick, broken PHAsset references) at the earliest point. The OCR pipeline never sees them. |
