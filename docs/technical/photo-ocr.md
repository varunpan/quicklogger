...
The container sets `BODY_SIZE_LIMIT=0` (adapter-node: no transport-layer body
cap). The sole size gate is `OCR_MAX_IMAGE_MB` (default 5 MiB → `env.ocrMaxImageBytes`),
...
bug; `0` removes the trap entirely and makes the app the single source of truth.
