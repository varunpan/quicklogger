...
`[Discard]` actions. Opt-in: at least one of the four provider slots
must be configured — `ollama-local` (`OLLAMA_VISION_URL`), `openrouter`
(`OPENROUTER_API_KEY`), `ollama-cloud` (`OLLAMA_CLOUD_API_KEY`), or
`openai-compatible` (`OPENAI_COMPATIBLE_URL` + `_API_KEY` + `_MODEL`);
otherwise the chips stay hidden. User guide:
...
- [`src/lib/server/env.ts`](../../src/lib/server/env.ts) — adds 27 OCR
  env vars, all optional with defaults. Feature activates iff any
  provider slot is configured (`OLLAMA_VISION_URL`,
  `OPENROUTER_API_KEY`, `OLLAMA_CLOUD_API_KEY`, or the
  `OPENAI_COMPATIBLE_URL`/`_API_KEY`/`_MODEL` trio).
- [`src/lib/server/ocr.ts`](../../src/lib/server/ocr.ts) —
  ...
  `selectProvider`'s process-level memoization of the `ocr chain
effective` log line is warmed at boot: `hooks.server.ts`'s
  `ensureBoot()` calls `selectProvider(env, getLogger())` once so the
  memoized line lands next to `server start` / `ocr_providers` instead
...
- [`src/lib/client/api.ts`](../../src/lib/client/api.ts) —
  ...
  `postOcr(image, mode, rotation, crop, lastOdometerMi,
lastPricePerUnit, timeoutMs)` (multipart POST; the client
  `AbortSignal.timeout` is the probe's `chainTimeoutMs + 10_000`,
  falling back to 90 s only when the probe supplied none).
...
- [`src/lib/client/CropOverlay.svelte`](../../src/lib/client/CropOverlay.svelte)
  ...
  (`floorSourcePx` is a required prop with no default — the host,
  `OcrPreview.svelte`, passes `200`; derived back into display px via the
  ...
  purpose: Svelte re-applies an _unbound_ bindable's fallback on every
...
- [`src/routes/+page.ts`](../../src/routes/+page.ts) — probes
  `GET /api/ocr` and surfaces `ocrEnabled` + `ocrModes` + `ocrChainTimeoutMs`
  to the page (see § Non-obvious decisions for how the timeout value is used).
...
| File                | Shape                                                                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ocr-budget.json`   | `{ date: 'YYYY-MM-DD', calls, costCents }`                               | UTC date; replaced (not appended) on each `add()`. The read-modify-write runs under a per-path lock and the file is written atomically (temp + `rename`) via [`atomicFile.ts`](../../src/lib/server/atomicFile.ts), so concurrent OCR calls can't lose an increment and a crash mid-write can't corrupt the tally. The _tally_ is exact; the _cap_ it feeds is advisory (see "Daily budget cap is advisory" below). |
...
1. Browser opens the OS chooser via `<input type="file" accept="image/*">`
   ...
   _Take Photo_ and _Photo Library_ options).
...
3. On `[Send for OCR]`, `resizeForOcr(file, { rotation, crop })` runs
   ...
   `postOcr(blob, mode, rotation, ...)` POSTs `multipart/form-data`
   with a client `AbortSignal.timeout` of the probe-reported
   `chainTimeoutMs + 10 s` (static 90 s only as fallback when the
   probe supplied none). The `rotation` form field is
...
4. Server: rate-limit check (in-memory sliding window, per-IP) →
   ...
   image size + magic-byte sniff. Adversarial / partial crop fields make
   `parseCropFields` return `null` (treated as un-cropped: `cropApplied: false,
cropRect: null`, see the edge-case table below) — nothing is zeroed; the OCR
   call still runs on whatever bytes the client actually sent.
...
6. Route handler increments budget, appends audit row, returns the
   discriminated `OcrResult`. On the _failure_ path the budget is also
...
7. Client: chip renders in the OCR feedback zone immediately under the
   ...
   _below_ last fillup → amber chip with `[Use anyway]` + `[Dismiss]`
...
```ts
{
  ...
  latencyMs: number,                          // runOcrPipeline start → outcome; excludes
                                              // rate-limit/budget checks, multipart parse,
                                              // and the audit write itself
...
2. `handlePumpCamera` buffers the pick into two independent Files via
   ...
   `prefillDateFromPhoto(exifFile)` is fired `void`-ed against the _other_
...
| Ollama transient outage with both configured                                           | Chain falls through to next configured slot; `fellbackFrom='ollama-local'` audited                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Bounded chain — each slot tried at most once per request                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Odometer jumped > 2000 (instance distance unit)                                        | No OCR-confirm chip — value flows to the blue suggestion; the `> 2000` jump is caught once, at submit, by smart-check E (`[Submit anyway]`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Was a redundant double-warning (#20b): the OCR-confirm chip and smart-check E both flagged the same `ODOMETER_MAX_DELTA` (renamed from `ODOMETER_MAX_DELTA_MI` in #69). E covers manual entry too, so it's the single gate. Hardcoded; promotable to Settings                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Network drops mid-OCR                                                                  | After the client timeout (`chainTimeoutMs + 10 s`; 90 s fallback): "OCR took too long" toast                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `AbortSignal.timeout` fires; no IDB queue (intentional — see SW doc)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| User commits crop, returns to preview, taps `[Rotate]`                                 | Rotate buttons work in preview mode; crop persists (it's in un-rotated source coords)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Sequential lock applies only _inside_ crop mode                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Zoom past 5× (pinch hard, or drag the slider to the end)                               | Capped at MAX_ZOOM=5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `clampZoom`; the zoom slider's `max` is `MAX_ZOOM`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
(`_contentLengthExceeds`, rejects before `formData()` buffers) and the
...
envelope vs. the decoded image). They are _not_ one value — the correct
...
Since v0.2.6 the fuelup multipart path also buffers image parts
(`pumpImage` / `odometerImage` for photo attach — see "Retained OCR blobs"
below), and `/api/fuelup` guards itself with the same early `Content-Length`
pre-guard pattern, capped at 2 × the image policy plus fixed form slack —
so dropping the global cap still exposes no unguarded route.
...
result flags only a _backwards_ reading (below last fillup) — advisory,
...
**Odometer _relative_ range lives client-side; _absolute_ range lives
server-side.** The server has no access to per-vehicle fillup history.
At OCR-confirm time `checkOdometerRelative` flags only a _backwards_
...
**`ODOMETER_MAX_DELTA = 2000` (renamed from `ODOMETER_MAX_DELTA_MI` in #69)
is hardcoded in client code, not env.** The bound is a UX safety net, not a
security gate, and the value is meaningful to the user (visible in the
warning copy). It's 2,000 in the instance distance unit — not converted, see
[`smart-checks.md`](./smart-checks.md#non-obvious-decisions). Promotable to
a Settings preference (alongside `odometerIncrementMi`) in a future v0.2.x
if real-world travel routinely hits 2000+ between fillups.
...
row lies — but the audit is for _our_ debugging, not adversarial trace.
...
original v0.2.0 design did `truncate(path, 0)`, which erased the _entire_
...
- **`set`-cue persists across OCR cancel / retake.** The EXIF date was
  extracted _before_ the preview mounted. If the user discards or retakes the
...
- Throw `OcrProviderError` with one of these codes on failure:
  ...
  - The chain catches **any** throw (not just `OcrProviderError`) and
    falls through; a non-`NETWORK` failure — including an unexpected
    non-`OcrProviderError` throw — is billed as received-response cost
    (the upstream answered, so tokens were burned).
...
- **Per-vehicle odometer delta.** `ODOMETER_MAX_DELTA=2000` is one
