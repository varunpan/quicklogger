...
| Safari (iOS **and** desktop) pump submission returns `400 multipart parse failed` | **Root cause (confirmed v0.2.4):** the server's `request.formData()` *throws* because the multipart body streams short on the wire — **not** because the `image` part is empty. (An empty part parses cleanly as a 0-byte `File` and yields the *different* error `empty image`; verified directly against undici.) In WebKit, when EXIF prefill and the OCR encode read the **same** `File`, the OCR-encoded image Blob reports a non-zero `size` but yields fewer bytes than its `Content-Length` when `fetch` serializes the body, so the parser hits EOF before the closing boundary. Pump-only because only pump fires date prefill. **Fix:** `bufferPickedPhoto` (`src/lib/client/photo-buffer.ts`) hands the OCR pipeline and the EXIF reader two **independent** Files with separate `ArrayBuffer` backing stores — the "independent in-memory copies" v0.2.3 documented but never actually implemented (it shared one buffered File between both paths). | **Supersedes the v0.2.3 diagnosis.** v0.2.3 read "zero bytes in the wire body" from Safari's network inspector — but Safari elides binary request bodies there, so a populated image part *looks* empty. v0.2.3 therefore hardened a `convertToBlob` zero-byte fallback and a PHAsset concurrent-read race that the evidence (undici parses empty parts fine; the bug reproduces deterministically on desktop, breaking the iOS-PHAsset premise) shows were never the trigger. Real-WebKit-only: not reproducible in jsdom or headless WebKit, so the regression guard (`photo-buffer.test.ts`) asserts the two-independent-Files invariant rather than the byte-corruption itself. The v0.2.3 `convertToBlob` fallback is retained as harmless defensive code. |
