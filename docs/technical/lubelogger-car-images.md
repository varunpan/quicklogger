...
- [`src/service-worker.ts`](../../src/service-worker.ts) — adds the fixed-name `IMG_CACHE` constant, a `staleWhileRevalidate` helper, a fetch-handler branch for `/api/vehicle/image` placed _before_ the generic `/api/` network-first branch, and a tweak to the activate handler so `IMG_CACHE` survives shell upgrades.
...
- [`src/routes/stats/+page.svelte`](../../src/routes/stats/+page.svelte) — vehicle card is the shared `<VehicleCard>`.
...
1. User opens any page that renders a vehicle row (`/`, `/history`, `/maintenance`, `/stats`, or the `/vehicles` picker list). Each rendered `<VehicleImage>` receives a `vehicleId` prop — either the active vehicle's id (single-vehicle surfaces) or the per-row id (picker).
...
3. The browser fires a GET. The service worker fetch handler matches `/api/vehicle/image` (matched _before_ the generic `/api/` branch) and routes through `staleWhileRevalidate`:
...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vehicleId` prop is `undefined` (no vehicle selected yet)    | SVG fallback renders directly — no `<img>`, no `/api/vehicle/image` request                                        | The render guard is `{#if vehicleImageOk && vehicleId !== undefined}` (`VehicleImage.svelte:25`); `vehicleId: number \| undefined` makes this a third, network-free path alongside the two `onerror` fallback cases below |
...
**Per-instance state, not a shared `Set<number>`.** The picker renders N rows, each with its own `<VehicleImage>` instance and therefore its own `vehicleImageOk` flag. A list-level `Set` of failed ids would centralize the bookkeeping but spread the lifecycle across the parent — keeping each row self-contained is simpler and matches the pattern used on the four single-vehicle surfaces (`/`, `/history`, `/maintenance`, `/stats`).
...
- **Multiple sizes / thumbnails.** The four single-vehicle surfaces already render at 48×48 (`VehicleCard.svelte`'s `w-12 h-12`) while the `/vehicles` picker renders at 56×56 (`w-14 h-14`, `svgSize={24}`) — both sizes are served from the same endpoint response, just scaled by CSS. Image bytes ~40 KB — no perf reason to add a server-side resize step for two sizes this close; a `?size=` query param could route through one if a much larger surface (e.g. a full-bleed vehicle-detail hero) is ever added.
