...
- `src/routes/api/vehicle/info/+server.ts` — `GET /api/vehicle/info?vehicleId=N`; a thin delegation to `parseVehicleId` + `withLubeLogger` (`src/lib/server/lubeloggerProxy.ts`), which validates the id and maps errors (502 on `LubeLoggerError`, 500 otherwise).
...
LubeLogger's `GET /api/vehicle/info` 1-element array. Five
`{category}RecordCount` / `{category}RecordCost` pairs (gas/service/repair/
upgrade/tax), `lastReportedOdometer`, four reminder-count buckets, and
`nextReminder: Reminder | null` (modelled but no longer rendered — see
_Non-obvious decisions_). `vehicleData` reuses the loose `Vehicle` type.
`plan*` counts exist upstream but are not modelled.
...
| `vehicleData.purchasePrice` absent / 0 / non-numeric / non-finite (e.g. `Infinity`) | Purchase-price line hidden                                                | `purchasePrice` guards `typeof === 'number' && Number.isFinite(p) && p > 0`       |
...
- **The reminder card shows counts only — no named "next" reminder.** It used to render `nextReminder.description` under the past-due badge, but LubeLogger's `nextReminder` is the next _upcoming_ item and skips past-due ones. With a brake-fluid reminder past due and an oil change due in months, the card read as "Engine Oil change — past due", which it wasn't. `/api/vehicle/info` only carries a past-due _count_, not the past-due reminder's name, so naming the real culprit would need a second `/api/vehicle/reminders` fetch that duplicates `/maintenance`. The compact card renders three independently-gated pieces, not one joined string: the `{pastDue} Past Due` badge (`+page.svelte:115-119`, only when `pastDue > 0`), the `{upcoming} upcoming` label (`:120-122`, only when `upcoming > 0`, with no `·` separator between the two — just flex-row spacing) and a static "View in Maintenance" line (`:124`) that always renders whenever the card itself is shown. The whole card links to `/maintenance` for the detail.
- **`upcoming` includes `NotUrgent`, but `/maintenance` filters it out.** `reminderSummary`'s `upcoming` is `veryUrgentReminderCount + urgentReminderCount + notUrgentReminderCount` (`src/lib/client/stats.ts:77-78`); `/maintenance`'s reminder list excludes `urgency === 'NotUrgent'` entirely (`src/routes/maintenance/+page.svelte:38`). A vehicle whose only reminders are `NotUrgent` can show "3 upcoming" on this card, then "Looks good — no upcoming maintenance" on `/maintenance` — the two pages can disagree about whether a vehicle has work due. This is the same misleading-but-faithful presentation class the decision above was written after — documented here as current behaviour. Reconciling the two counts would be a product decision (which reminders "count" as work due), not a doc fix; out of scope here.
