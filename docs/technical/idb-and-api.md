...
| `deduped`    | `boolean`                            | yes      | Literal `true` when the replay response carried `deduped: true` (record already upstream, write skipped). A flag on a still-`'synced'` row, deliberately not a fourth `status` value — `pruneSynced` and the `byStatus` index both key on `'synced'`. Renders as a muted **Skipped** badge on `/history`. |
...
| `markSynced(id, converted?, deduped?)` | Set status `'synced'`; store `converted` when provided; set `deduped: true` when the third argument is truthy (never written as `false`). No-op if id missing.                                            |
