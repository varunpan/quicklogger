...
- On network failure, return a `504` with body `'offline'` — control only
  reaches the catch when `cached` was `undefined` (a cache hit returned
  earlier), so the 504 is what the user gets when both cache and network
  fail.
