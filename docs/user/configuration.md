...
| `LUBELOGGER_VOLUME_UNIT`       | `gallons_us` \| `liters` | no       | `gallons_us`                        | The volume unit your LubeLogger instance stores fuel in. Imperial Calculation checked → `gallons_us`; metric/UK instances → `liters`. Any other value fails at startup with `EnvError`.                                        |
| `LUBELOGGER_DISTANCE_UNIT`     | `miles` \| `km`          | no       | `miles`                             | The distance unit your LubeLogger instance tracks odometers in. Drives every distance label in the app (history, stats, maintenance, smart-check messages, photo filenames). Any other value fails at startup with `EnvError`. |
...
Both `gallons_us` and `liters` are supported (since v0.3.2). Set this
to **match your LubeLogger instance's own volume unit** — quicklogger
cannot auto-detect it, because LubeLogger's `/api/info` exposes no
unit fields. A mismatch doesn't error; it silently writes
wrong-magnitude numbers upstream (e.g. entering liters while
quicklogger believes the instance is `gallons_us` inserts a number
~3.8x too small).

The form's own `Gal`/`L` toggle is for input convenience only; the
server always converts the entered volume to whatever unit
`LUBELOGGER_VOLUME_UNIT` specifies before posting.

#### `LUBELOGGER_DISTANCE_UNIT`

Labels only — odometer values pass through unconverted regardless of
this setting. It controls how distances are displayed (history,
stats, maintenance, smart-check messages) and the suffix used in
saved photo filenames, e.g. `odometer-12345km.jpg`.
