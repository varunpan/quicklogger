...
Volume and distance figures throughout this page — chips, previews,
history, stats, maintenance — follow your LubeLogger instance's
configured units (gallons or liters; miles or km). Examples below use
the defaults (`Gal`, `mi`); see [`units.md`](units.md) for the full
picture.
...
| **Missed fillup**             | Toggle button. Blue when on. Marks the entry as one where a previous fillup was missed (so LubeLogger's fuel-economy calcs handle the gap).                                                                                                                                                                                                                                                                    |
| **Note · station · grade**    | Free-form text. Plain string, sent to LubeLogger's `notes` field.                                                                                                                                                                                                                                                                                                                                              |
| **`+N mi`/`+N km` this tank** | Read-only line below the odometer, shown once you've entered a new reading. The blue number is how far you've driven since the previous fillup (this reading minus the last), in your instance's distance unit.                                                                                                                                                                                                |
| **Will log** preview          | Blue summary line appearing when both volume and cost are valid. Shows the converted volume (gal or L, matching your instance) plus the cost in your LubeLogger instance's currency (it falls back to USD only on a cold cache), plus a fuel-economy figure since last fill when computable — MPG on a gal+mi instance, L/100km on a L+km instance; hidden on mixed unit pairings. See [`units.md`](units.md). |
...
| **Odometer prefill**           | `On` / `Off` toggle. Off = form opens with an empty odometer field; the quick-increment chip is also hidden.                                                                                                                    | `odometerPrefillEnabled` (bool, default `true`) |
| **Quick increment (mi or km)** | Number input; label reads your instance's distance unit. The `+N` chip below the odometer field adds this many units per tap. Set to `0` to hide the chip while keeping the prefilled value.                                    | `odometerIncrementMi` (int, default `300`)      |
